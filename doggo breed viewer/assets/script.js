...
fetch(breedsUrl)
    ...
    .then(data => {
        const breedsObj = data.message; // turn the message into the object
        const breedsArray = Object.keys(breedsObj) // turn the object into an array
        console.log(breedsObj)
        for (let i = 0; i < breedsArray.length; i++) {
            const option = document.createElement("option")
            option.value = breedsArray[i] // sets the value of the option
            option.innerText = breedsArray[i] // displays the value breed in the dom
    ...
    function getDoggoImg(url) {
        fetch(url) //going to the API url above
            .then(res => {
                return res.json(); //get JSON message back
            })
            .then(data => {
                img.src = data.message // assign message from json source to the image source
            })
    }
    ...
    const doggoInfo = {
        ...
        zodiacList: ['Pisces', 'Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpio', 'Sagittarius', 'Aquarius'],
        ...
        sneakFoodList: ['Yes, but isn\'t very sneaky', 'Would\'t dare', 'Without a doubt', 'No, but will stare at you while you eat', 'Will take food right from your fork if given the chance'],
        ...
        assignAge() {
            this.age = Math.floor(Math.random() * 16 + 1) // rounds down, picks a random number from 16 and plus 1 gives you a minimum value
        ...
        assignLikes() {
            this.likes = this.yatesShuffle(this.likesList);
            document.getElementById("likes").innerHTML = `Likes: ${this.likes[0]}, ${this.likes[1]}`
        },

        assignDislikes() {
            this.dislikes = this.yatesShuffle(this.dislikesList);
